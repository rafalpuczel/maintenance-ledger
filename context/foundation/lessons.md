...
## Vitest has no `@/` path alias — import a tested module's siblings relatively

- **Context**: Adding a `*.test.ts` for a `src/lib/<domain>/` module (e.g. a `form.test.ts` for `form.ts`). `vitest.config.ts` defines only `test.include` + `environment` — **no `resolve.alias`** for the `@/` TS path that astro/vite resolve in the app build.
- **Problem**: In report-authoring, `src/lib/reports/form.ts` imported its sibling schema as `@/lib/reports/schema`. The app `build` and `astro check` resolved it fine (vite/tsconfig know `@/`), so it looked correct — but `npm test` failed at collection with "Cannot find package '@/lib/reports/schema'" the moment `form.test.ts` pulled `form.ts` → schema through vitest. The existing domain modules already dodge this: `plugins-catalog/form.ts` imports `./schema`, not `@/...`.
- **Rule**: Within a `src/lib/<domain>/` folder, import siblings **relatively** (`./schema`, `./queries`) — never via the `@/` alias — so the module stays resolvable under vitest. Reserve `@/...` for cross-domain imports (e.g. `@/lib/plugins-catalog/queries` from another domain), which tests don't pull. A green `build`/`astro check` does NOT imply a green `npm test`; run the suite too when adding code under `src/lib/`.
- **Applies to**: implement, plan, impl-review

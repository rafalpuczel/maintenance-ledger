...
- **Który obszar przeanalizować (L3).** Mapa pokazała media-editor jako strefę ryzyka #3, więc na tym zrobiłem research. Na pierwszy rzut oka wygląda niegroźnie, ale historia zmian i niewielka liczba osób pracujących przy tym kodzie sugerowały coś innego.
- **Jak wąski ma być plan (L4).** Zostawiłem tylko część dotyczącą glue z opcji #1. Świadomie odrzuciłem modal-contract i save-format, bo są związane z kontraktem REST WordPressa i architekturą warstw. Próba poprawiania ich "przy okazji" tylko niepotrzebnie zwiększyłaby zakres i ryzyko zmiany.
- **Który niezmiennik jest najważniejszy (L5).** Skupiłem się na Report Send Log, nie na No-leak. Oba są istotne, ale No-leak jest już dobrze zabezpieczony przez istniejącą strukturę kodu. Z kolei poprawność send-log zależy praktycznie od kolejności obsługi błędów w jednej ścieżce wykonania, więc uznałem ją za bardziej podatną na regresje.
- **Gdzie celowo odszedłem od PRD.** Brak optimistic UI (US-03) i brak osobnej akcji download (S-12) nie były przeoczeniem. To celowe decyzje wynikające z oceny kosztu, ryzyka i wartości tych zmian.

I jedna lekcja na przyszłość: nie ufam zeru z ast-grepa - potwierdzam `grep`em, bo raz pokazało zero, którego nie było (korekta C6).
